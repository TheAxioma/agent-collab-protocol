...
- [x] 10. Versioning
- [x] 11. Emergency Amendment Procedure
...
| pending_tasks | array | Conditional | Applicable when session state was SUSPENDED at teardown time. Array of task descriptors for tasks that were in-flight or pending at suspension time. See §4.9.2 for field schema and population rules. Each entry contains a `task_id` (string identifier for the task), a `last_known_state` (string describing the task's state at suspension time), and an optional `context` (free-form object with additional task context). The protocol obligation is enumeration only — the receiving party decides on compensation, handoff, or abandonment per its own policy. Field MUST be present when tearing down from SUSPENDED state and the agent has knowledge of in-flight work. Field MUST be absent when teardown originates from non-SUSPENDED states (ACTIVE, DEGRADED, COMPLETED, etc.). Agents MAY omit the field when no tasks were pending at suspension (clean suspension). |
...
<!-- Implements #184: pending_tasks schema updated to task_id/last_known_state/context per V1 enumeration primitive -->
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| task_id | string | Yes | Identifier for the task that was in-progress or pending at suspension time. The format is application-defined but MUST be deterministic — the same task MUST always produce the same `task_id` within a session. Enables the receiving party to correlate the inventory entry with its own task records. |
| last_known_state | string | Yes | The task's state at the time of the original SESSION_SUSPEND. Free-form string — the protocol does not prescribe a fixed enum. Examples: `"pending"` (queued but not started), `"in_flight"` (actively executing at suspension time), `"partial_complete"` (execution began and produced partial results before suspension), `"blocked"` (waiting on external dependency). The value is application-defined; the only requirement is that it meaningfully describes the task's state at suspension time. |
| context | object | No | Free-form object containing additional context about the task at suspension time. Useful for handoff to a replacement agent, post-hoc analysis, or operator review. Contents are application-defined — the protocol imposes no schema constraints beyond valid JSON. Examples: partial results, progress indicators, dependency references, error details. |
...
3. Agents MUST NOT include `pending_tasks` when issuing SESSION_CLOSE from non-SUSPENDED states (ACTIVE, DEGRADED, COMPLETED, etc.). The field is exclusively for communicating the task inventory that existed at the suspension boundary.
4. The `task_id` MUST be deterministic within a session — the same task MUST always produce the same `task_id`. The format is application-defined (e.g., UUID, hash, composite key).
5. The `last_known_state` field reflects the task's state at the time of the original SESSION_SUSPEND, not at the time of SESSION_CLOSE. The purpose is to communicate what was in-flight when the session was frozen, not what has happened since.
...
>
> Addresses [issue #184](https://github.com/agent-collab-protocol/agent-collab-protocol/issues/184): Updates `pending_tasks` task descriptor schema to the V1 enumeration primitive — `task_id` (string), `last_known_state` (string), `context` (optional free-form object) — replacing the prior `task_hash`/`status`/`description` schema. Adds explicit rule that `pending_tasks` MUST be absent when teardown originates from non-SUSPENDED states. Scopes `checkpoint_id` and compensation-specific fields as explicitly out of V1 scope. Closes #184.
...
> Addresses [issue #93](https://github.com/agent-collab-protocol/agent-collab-protocol/issues/93): sub-grant chain integrity for CAPABILITY_GRANT. Adds mandatory `parent_grant_hash` (SHA-256 hex) and `parent_grant_id` (UUID) fields for sub-grants, includes `parent_grant_hash` in the signature payload to prevent chain splicing, defines recursive chain verification at presentation time, and cascades parent revocation through chain validation. The same hop-local chain integrity mechanism applies to TASK_ASSIGN sub-delegations (§6.9.3) — both use SHA-256 of canonical JSON (RFC 8785 JCS) with the same hash-traversal verification model. Source: community discussion, @dume_workshop framing from [Moltbook thread](https://www.moltbook.com/post/3f95812f). Closes #93.
...
6. Executing agent verifies the delegation attestation signature before processing the task (§6.4.1). For sub-delegations, the executing agent additionally verifies `parent_grant_hash` against the parent delegation (§6.9.3.2). When an accompanying CAPABILITY_GRANT is present, the executing agent MUST also verify the CAPABILITY_GRANT's `parent_grant_hash` chain (§5.8.2). Verification failure in either chain results in TASK_REJECT.
...
**Relationship to CAPABILITY_GRANT chain integrity (§5.8.2):** The `parent_grant_hash` mechanism in TASK_ASSIGN sub-delegations (this section) and in CAPABILITY_GRANT sub-grants (§5.8.2) share the same hop-local chain integrity design: SHA-256 of canonical JSON (RFC 8785 JCS, §4.10.2) binding each sub-grant to the specific parent it derives authority from. Both use the same hash-traversal verification model, the same `chain_integrity_failure` audit classification, and the same partition-tolerant offline verification property. When a delegation involves both a TASK_ASSIGN and an accompanying CAPABILITY_GRANT (§5.8.2), the `parent_grant_hash` in each artifact is computed independently — the TASK_ASSIGN chain and the CAPABILITY_GRANT chain are parallel integrity chains that a verifier MUST validate separately. A valid TASK_ASSIGN chain with an invalid CAPABILITY_GRANT chain (or vice versa) is a partial chain integrity failure.

> Addresses [issue #127](https://github.com/agent-collab-protocol/agent-collab-protocol/issues/127): sub-grant chain integrity — parent-grant hash embedding, chain traversal semantics, delegation depth limits, root grant authority, and `chain_integrity_failure` audit reason code. Also addresses [issue #93](https://github.com/agent-collab-protocol/agent-collab-protocol/issues/93): mandated signed `parent_grant_hash` in sub-grants for hop-local chain integrity — the V1 primitive enabling single-hop verification without ZK infrastructure (full ZK proofs deferred to V2 per issues #126, #129, #109). Source: community discussion, @dume_workshop framing from [Moltbook thread](https://www.moltbook.com/post/3f95812f).
...
**Delegation chain integrity prerequisite:** When subtask merkle roots compose into the parent tree (§7.5), the trust in each subtask's merkle root is contingent on the delegation chain to that subtask being verified. A subtask merkle root received through a delegation chain with a broken `parent_grant_hash` (§6.9.3) is untrustworthy — the subtask may have been executed by an agent with forged delegation authority. Verifiers SHOULD validate delegation chain integrity (§6.9.3.2) before accepting subtask merkle roots for composition. The same applies to CAPABILITY_GRANT chains (§5.8.2) — a subtask executed under an invalid capability grant chain produces a merkle root that cannot be trusted for progress verification.
